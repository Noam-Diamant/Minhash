...
################################################################################
if {[::legacy::find -design design:proj_hasher] ne ""} {
  puts "** A design with the same name is already loaded. It will be removed. **"
  delete_obj design:proj_hasher
}
...
# Design
################################################################################
read_netlist -top proj_hasher /data/project/tsmc65/users/noaant/ws/Minhash/synthesis/workspace/../export/post_synth/proj_hasher.v
...
phys::read_script /data/project/tsmc65/users/noaant/ws/Minhash/synthesis/workspace/../export/post_synth/proj_hasher.g
...
# slack
set _slk_ [::legacy::get_attribute slack design:proj_hasher]
if {[regexp {^-?[0-9.]+$} $_slk_]} {
  set _slk_ [format %.1f $_slk_]
}
if {$_slk_ != "-2033.4"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden slack: -2033.4,  current slack: $_slk_"
}
unset _slk_
# multi-mode slack
# tns
set _tns_ [::legacy::get_attribute tns design:proj_hasher]
if {[regexp {^-?[0-9.]+$} $_tns_]} {
  set _tns_ [format %.0f $_tns_]
}
if {$_tns_ != "45418"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden tns: 45418,  current tns: $_tns_"
}
unset _tns_
# cell area
set _cell_area_ [::legacy::get_attribute cell_area design:proj_hasher]
if {[regexp {^-?[0-9.]+$} $_cell_area_]} {
  set _cell_area_ [format %.0f $_cell_area_]
}
if {$_cell_area_ != "9086"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden cell area: 9086,  current cell area: $_cell_area_"
}
unset _cell_area_
# net area
set _net_area_ [::legacy::get_attribute net_area design:proj_hasher]
if {[regexp {^-?[0-9.]+$} $_net_area_]} {
  set _net_area_ [format %.0f $_net_area_]
}
if {$_net_area_ != "2528"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden net area: 2528,  current net area: $_net_area_"
}
unset _net_area_
